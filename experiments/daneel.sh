...
#
#   sh experiments/daneel.sh defmap        start or resume the Def-MAP probes
#   sh experiments/daneel.sh umx           the Open-Unmix baseline
#   SLICES="0:20 20:30 30:40 40:50" LOG=/scratch/logs/lot3 sh experiments/daneel.sh defmap
#     four shards of the same plan, one container each. MAX_TRAIN_FRAMES and
#     TEST_SECONDS are forwarded rather than left at the `ceiling_sweep` defaults
#     of 20000 and five seconds, those defaults being the seven-second edition's
#     and not the published arm's, and MUSDB has to name the full database for
#     the same reason. A cell run at the defaults is silently a different
#     experiment.
#   MUSDB_TEST=$HOME/data/gasm-demos/musdb18 TEST_OFFSETS=0,30,60 ... defmap
...
if [ -z "$SLICES" ]; then
    launch "defmap-$PLAN" "$LOG" ""
else
    i=0
    for slice in $SLICES; do
        launch "defmap-$PLAN-s$i" "$LOG/s$i" "$slice"
        i=$((i + 1))
    done
fi
